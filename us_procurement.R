...
setwd("~/Downloads")
library(readxl)
df_procurement <- read_excel("us_procurement_april2024.xlsx")
df_celi <- read.csv(file = 'yale_celi.csv')
df_procurement <- read.csv(file = 'us_procurement_april2024.csv')

library(dplyr)
library(tidyr)

# Assuming df_procurement is your data frame and KSEstatus is the column of interest
df_procurement <- df_procurement %>%
  mutate(exit = ifelse(KSEstatus %in% c("leave", "exited"), 1, 0))
...
df_procurement <- df_procurement %>%
  mutate(award21 = as.numeric(as.character(award21))) %>%
  mutate(award21 = case_when(
    is.na(award21) | award21 == 0.0 ~ 0,  # Handle NA and 0.0 as 0
    award21 == 1.0 ~ 1,                   # Handle 1.0 as 1
    TRUE ~ award21                        # Keep other values as they are
  ))


model1 <- glm(exit~ award21 ,
...
df_procurement <- df_procurement %>%
  select('Company.name', exit, award21, contract21, revRF21, capRF21, globalrev21, numlocationRF)
...
df_procurement <- df_procurement %>%
  mutate(
    contract21 = as.numeric(gsub("[, ]", "", as.character(contract21))), 
    revRF21 = as.numeric(gsub("[, ]", "", as.character(revRF21))),          # Remove commas and spaces
    capRF21 = as.numeric(gsub("[, ]", "", as.character(capRF21))),          # Remove commas and spaces
    globalrev21 = as.numeric(gsub("[, ]", "", as.character(globalrev21))),  # Remove commas and spaces
    numlocationRF = as.numeric(gsub("[, ]", "", as.character(numlocationRF))) # Remove commas and spaces
  )

df_procurement <- df_procurement %>%
  mutate(numlocationRF = if_else(is.na(numlocationRF), 0, numlocationRF))


df_procurement <- df_procurement %>%
  mutate(
    contract_ratio = if_else(globalrev21 == 0 | is.na(globalrev21), NA_real_, contract21 / globalrev21)
  )


df_procurement <- df_procurement %>%
  mutate(
    revRF21 = if_else(revRF21 == 0, 1e-6, revRF21),
    capRF21 = if_else(capRF21 == 0, 1e-6, capRF21),
    globalrev21 = if_else(globalrev21 == 0, 1e-6, globalrev21)
  ) %>%
  mutate(
    log_revRF21 = log(revRF21),
    log_capRF21 = log(capRF21),
    log_globalrev21 = log(globalrev21),
  )
...
df_procurement <- df_procurement %>%
  mutate(
    numlocationRF = if_else(is.na(numlocationRF), 0, numlocationRF)  # Replace NA with 0
  )
...
model2 <- glm(exit~ award21 + log_globalrev21 + log_revRF21 + log_capRF21 + numlocationRF,
              family=binomial(link="logit"),
              data = df_procurement)
...
model1 <- glm(exit~ contract_ratio,
              family=binomial(link="logit"),
              data = df_procurement)
...
model2 <- glm(exit~ contract_ratio + log_globalrev21 ,
              family=binomial(link="logit"),
              data = df_procurement)
...
m1_alt <- glm(exit2~ award21 + log_globalrev21 + log_staff21 + sharerevRF21 + numlocalRU,
              family=binomial(link="logit"),
              data = df)
